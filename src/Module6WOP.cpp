...
Module6WOP* Module6WOP::New(physx::PxPhysics* p, physx::PxScene* s, const std::string& path, Aftr::Vector scale, Aftr::MESH_SHADING_TYPE mst, bool stat, physx::PxVec3 pos)
{
	Module6WOP* wo = new Module6WOP(p, s);
	wo->onCreate(path, scale, mst, stat, pos);
	return wo;
}

Module6WOP::Module6WOP(physx::PxPhysics* p, physx::PxScene* s) : IFace(this), WO()
{
	this->p = p;
	this->s = s;
	this->a = nullptr;
...
void Module6WOP::onCreate(const std::string& path, const Aftr::Vector& scale, Aftr::MESH_SHADING_TYPE mst, bool stat, physx::PxVec3& pos)
{
...
PxActor* Module6WOP::getPxActor(physx::PxPhysics* p)
{
...
void Module6WOP::updatePoseFromPhysicsEngine(physx::PxActor* actor)
{
...
void Module6WOP::setPosition(Aftr::Vector v)
{
	WO* wo = this->getWO();
